...
echo "$($PSQL "SELECT name FROM teams LEFT JOIN (SELECT winner_id FROM games WHERE year = 2014 AND round = 'Eighth-Final') AS games ON teams.team_id = games.winner_id LEFT JOIN (SELECT opponent_id FROM games WHERE year = 2014 AND round = 'Eighth-Final') AS g ON teams.team_id = g.opponent_id WHERE games.winner_id IS NOT NULL OR g.opponent_id IS NOT NULL ORDER BY name")"
